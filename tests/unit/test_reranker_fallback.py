...
# =============================================================================
# Test: Fallback observability
# =============================================================================

class TestFallbackObservability:
    """A fallback that leaves no trace makes the degradation rate unmeasurable."""

    def test_fallback_records_rerank_stage(self, mock_settings, sample_results):
        """Failing reranker must still record a rerank stage, flagged as fallback."""
        from src.core.trace.trace_context import TraceContext

        trace = TraceContext()
        reranker = CoreReranker(
            settings=mock_settings,
            reranker=FailingReranker(error_message="backend exploded"),
        )

        reranker.rerank("test query", sample_results, trace=trace)

        stages = [s for s in trace.stages if s["stage"] == "rerank"]
        assert len(stages) == 1, "fallback path must record the rerank stage"
        data = stages[0]["data"]
        assert data["fallback"] is True
        assert "backend exploded" in data["fallback_reason"]
        assert data["input_count"] == 3
        assert data["output_count"] == 3
        assert stages[0]["elapsed_ms"] >= 0.0

    def test_success_path_has_no_fallback_flag(self, mock_settings, sample_results):
        """The success path records the same stage name but without the flag."""
        from src.core.trace.trace_context import TraceContext

        trace = TraceContext()
        reranker = CoreReranker(settings=mock_settings, reranker=FakeReranker())

        reranker.rerank("test query", sample_results, trace=trace)

        stages = [s for s in trace.stages if s["stage"] == "rerank"]
        assert len(stages) == 1
        assert "fallback" not in stages[0]["data"]
