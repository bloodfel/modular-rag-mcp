...
class TestLLMRerankerUsageMeta:
    """Per-call token usage metadata attached to reranked candidates."""

    def test_rerank_attaches_llm_usage_meta(self, mock_settings, sample_prompt, sample_candidates, tmp_path):
        prompt_file = tmp_path / "rerank.txt"
        prompt_file.write_text(sample_prompt)

        llm_response = json.dumps([{"passage_id": "chunk_1", "score": 3}])
        reranker = LLMReranker(
            settings=mock_settings,
            prompt_path=str(prompt_file),
            llm=MockLLM(response_content=llm_response),
        )

        result = reranker.rerank("query", sample_candidates)

        assert len(result) > 0
        assert all("_llm_meta" in c for c in result)
        meta = result[0]["_llm_meta"]
        assert meta["tokens_in"] == 10
        assert meta["tokens_out"] == 10
        assert meta["model"] == "mock-model"

    def test_usage_meta_absent_when_llm_reports_no_usage(self, mock_settings, sample_prompt, sample_candidates, tmp_path):
        prompt_file = tmp_path / "rerank.txt"
        prompt_file.write_text(sample_prompt)

        class NoUsageLLM(MockLLM):
            def _chat(self, messages, **kwargs):
                super()._chat(messages, **kwargs)
                return ChatResponse(content=self.response_content, model="mock-model", usage=None)

        llm_response = json.dumps([{"passage_id": "chunk_1", "score": 3}])
        reranker = LLMReranker(
            settings=mock_settings,
            prompt_path=str(prompt_file),
            llm=NoUsageLLM(response_content=llm_response),
        )

        result = reranker.rerank("query", sample_candidates)
        assert all("_llm_meta" not in c for c in result)
