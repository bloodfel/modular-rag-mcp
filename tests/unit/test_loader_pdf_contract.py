...
class TestImageExtractionDegradation:
    """Image extraction must record WHY it degraded, not fail silently.

    Regression: a Streamlit process started before pymupdf was installed kept
    its import-time PYMUPDF_AVAILABLE=False forever, silently ingesting
    text-only — and nobody could tell why captions were missing.
    """

    def test_missing_pymupdf_stamped_in_metadata(self, monkeypatch, tmp_path):
        """Without pymupdf, load() stamps image_extraction with the reason."""
        import src.libs.loader.pdf_loader as pdf_loader_module

        if not SIMPLE_PDF.exists():
            pytest.skip(f"Test fixture not found: {SIMPLE_PDF}")

        monkeypatch.setattr(pdf_loader_module, "PYMUPDF_AVAILABLE", False)
        loader = PdfLoader(image_storage_dir=tmp_path)
        doc = loader.load(SIMPLE_PDF)

        assert doc.metadata.get("image_extraction") == "pymupdf_not_installed"
        assert "images" not in doc.metadata

    def test_successful_extraction_not_stamped(self, tmp_path):
        """A normal load must not carry an image_extraction failure stamp."""
        if not IMAGES_PDF.exists():
            pytest.skip(f"Test fixture not found: {IMAGES_PDF}")
        ...
        assert "image_extraction" not in doc.metadata
