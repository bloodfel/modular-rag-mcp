...
# ── Collector → Langfuse fan-out ─────────────────────────────────────

class TestCollectorLangfuseFanout:
    """collect() is the single place a trace lands, so it owns both sinks."""

    def test_collect_sends_the_record_to_langfuse(self, tmp_path, monkeypatch) -> None:
        sent = []
        monkeypatch.setattr(
            "src.core.trace.trace_collector.send_trace",
            lambda record: sent.append(record) or True,
        )
        collector = TraceCollector(traces_path=tmp_path / "traces.jsonl")
        tc = TraceContext(trace_type="query")
        tc.record_stage("fusion", {"result_count": 3})
        tc.finish()

        collector.collect(tc)

        assert len(sent) == 1, "every collected trace is reported"
        assert sent[0]["trace_id"] == tc.trace_id
        assert sent[0]["stages"][0]["stage"] == "fusion"

    def test_local_file_is_written_even_if_langfuse_fails(self, tmp_path, monkeypatch) -> None:
        """The JSONL is the record of truth; Langfuse is best-effort."""

        def boom(record):
            raise RuntimeError("langfuse down")

        monkeypatch.setattr("src.core.trace.trace_collector.send_trace", boom)
        p = tmp_path / "traces.jsonl"
        collector = TraceCollector(traces_path=p)
        tc = TraceContext()
        tc.finish()

        collector.collect(tc)  # must not raise

        assert json.loads(p.read_text().strip())["trace_id"] == tc.trace_id

    def test_send_trace_is_a_noop_without_credentials(self, monkeypatch) -> None:
        from src.observability import langfuse_sink

        monkeypatch.delenv("LANGFUSE_PUBLIC_KEY", raising=False)
        monkeypatch.delenv("LANGFUSE_SECRET_KEY", raising=False)

        assert langfuse_sink.langfuse_configured() is False
        assert langfuse_sink.send_trace({"trace_id": "t1"}) is False

    def test_send_trace_swallows_network_errors(self, monkeypatch) -> None:
        """An unreachable Langfuse must never propagate into the traced work."""
        from src.observability import langfuse_sink
        ...
        monkeypatch.setattr(langfuse_sink, "_post_otlp", unreachable)

        record = {"trace_id": "t1", "started_at": "2026-09-22T00:00:00+00:00", "stages": []}
        assert langfuse_sink.send_trace(record) is False
