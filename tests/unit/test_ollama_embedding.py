...
def mock_ollama_response() -> dict[str, Any]:
    """Create a mock Ollama /api/embed response."""
    return {
...
class TestOllamaEmbedding:
    ...
    def test_embed_single_text(
        ...
        # Verify API call: batch endpoint with truncation (the legacy
        # /api/embeddings endpoint 500s when a text exceeds model context)
        ...
        assert call_args[1]["json"]["truncate"] is True
    ...
    def test_embed_multiple_texts(
        ...
        """Test embedding multiple texts in one batch request."""
        # Setup mock HTTP response with one embedding per input text
        mock_response = Mock()
        ...
        # Assert: order preserved, single batch call
    ...
    @patch('httpx.Client')
    def test_embed_count_mismatch_rejected(
        self,
        mock_client_class: Mock,
        mock_settings_ollama: Any,
    ) -> None:
        """A response with fewer vectors than inputs must fail loudly: callers
        pair vectors with texts by position, so a mismatch would silently
        ...
        with pytest.raises(OllamaEmbeddingError, match="returned 1 embeddings for 2 input texts"):
            embedding.embed(["hello world", "test"])
    ...
    @patch('httpx.Client')
    def test_embed_missing_embeddings_field(
        self,
        mock_client_class: Mock,
        mock_settings_ollama: Any,
    ) -> None:
        """Test handling of response missing 'embeddings' field."""
