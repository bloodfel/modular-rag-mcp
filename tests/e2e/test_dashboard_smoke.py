...
class TestDashboardSmoke:
    ...
    # ------------------------------------------------------------------
    # 2.5 Query Playground page
    # ------------------------------------------------------------------

    @pytest.mark.e2e
    def test_query_playground_page_renders(self) -> None:
        """Query Playground page loads and shows its input controls."""
        from streamlit.testing.v1 import AppTest

        mock_svc = MagicMock()
        mock_svc.list_collections.return_value = ["default"]

        def page_script():
            from src.observability.dashboard.pages.query_playground import render
            render()

        at = AppTest.from_function(page_script, default_timeout=10)

        with patch(
            "src.observability.dashboard.pages.query_playground.DataService",
            return_value=mock_svc,
        ), patch(
            "src.observability.dashboard.pages.query_playground.load_settings",
            return_value=_mock_settings(),
        ):
            at.run()

        assert not at.exception, (
            f"Query Playground page raised an exception: {at.exception}"
        )
        text = _collect_text(at)
        assert "query" in text.lower() and "playground" in text.lower()
