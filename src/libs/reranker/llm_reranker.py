    ...
    def rerank(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            response = self.llm.chat(messages, trace=trace, purpose="rerank", **kwargs)
        ...
        llm_meta = self._extract_usage_meta(response)
        ...
        # Every reranked candidate carries the same per-call usage metadata
        # (mirrors JevReranker._jev_meta so the benchmark can price this column).
        if llm_meta:
            for candidate in reranked:
                candidate["_llm_meta"] = dict(llm_meta)

        return reranked

    @staticmethod
    def _extract_usage_meta(response: Any) -> Dict[str, Any]:
        """Extract token usage metadata from a ChatResponse.

        Args:
            response: ChatResponse returned by the LLM provider.

        Returns:
            Dict with tokens_in/tokens_out/model, or {} when usage is absent.
        """
