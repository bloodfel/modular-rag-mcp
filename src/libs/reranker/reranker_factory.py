...
class RerankerFactory:
    ...
    def create(cls, settings: Settings, **override_kwargs: Any) -> BaseReranker:
        ...
        # Lazy register Jev (TypeSafe System One) reranker if not already registered
        if "jev" not in cls._PROVIDERS:
            JevReranker = _lazy_import_jev_reranker()
            cls.register_provider("jev", JevReranker)
