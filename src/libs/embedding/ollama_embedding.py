...
class OllamaEmbedding(BaseEmbedding):
    ...
    def embed(
        ...
        **kwargs: Any,
        ...
        Uses the batch ``/api/embed`` endpoint with ``truncate: True``: the
        legacy ``/api/embeddings`` endpoint has no truncation and fails with
        HTTP 500 once a text exceeds the model's context window, which is how
        LLM-refined chunks (refinement + captions inflate the text) used to
        break ingestion.
        ...
        try:
            with httpx.Client(timeout=self.timeout) as client:
                ...
                # Callers pair vectors with texts by position (batch i-th text
                # -> i-th vector), so a short/long response must not pass.
                if len(embeddings) != len(texts):
                    raise OllamaEmbeddingError(
                        f"Ollama API returned {len(embeddings)} embeddings "
