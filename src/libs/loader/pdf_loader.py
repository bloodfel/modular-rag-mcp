    ...
    def load(self, file_path: str | Path) -> Document:
        ...
        if self.extract_images:
            if not PYMUPDF_AVAILABLE:
                logger.warning(f"PyMuPDF not available, skipping image extraction for {path}")
                # Recorded so the trace/dashboard can say WHY images are
                # missing — a running process never re-checks this import.
                metadata["image_extraction"] = "pymupdf_not_installed"
            else:
                try:
                    ...
                    metadata["image_extraction"] = f"failed: {e}"
