...
This evaluator computes simple, deterministic metrics such as hit rate, MRR and
nDCG.  It is designed for fast regression checks and sanity validation.
"""
...
from src.observability.evaluation.metrics import ndcg_at_k


class CustomEvaluator(BaseEvaluator):
    """Custom evaluator for lightweight metrics (hit_rate, mrr, ndcg).

    The evaluator expects retrieved chunks to contain an identifier field.
    Supported id fields: id, chunk_id, document_id, doc_id.
    """

    SUPPORTED_METRICS = {"hit_rate", "mrr", "ndcg"}
    DEFAULT_NDCG_K = 10
    ...
    def evaluate(
        ...
        **kwargs: Any,
        ...
        if "ndcg" in self.metrics:
            results["ndcg"] = self._compute_ndcg(retrieved_ids, ground_truth_ids)
    ...
    def _extract_ids(self, items: Iterable[Any], label: str) -> List[str]:
        ...
        for index, item in enumerate(items):
            ...
            if hasattr(item, "id"):
                ...
                continue
            # Objects follow the same field list as dicts: the retriever's
            # RetrievalResult carries chunk_id, not id.
            for field in self._ID_FIELDS:
    ...
    def _compute_ndcg(
        ...
        """Compute nDCG@k, treating every expected id as equally relevant.

        The golden set gives a flat list of expected ids rather than graded
        relevance, so all of them get relevance 1; the ideal ranking is then
        simply "all expected ids first".
        """
        if not ground_truth_ids:
            return 0.0
        qrels = {chunk_id: 1 for chunk_id in ground_truth_ids}
        return ndcg_at_k(retrieved_ids, qrels, k or self.DEFAULT_NDCG_K)
