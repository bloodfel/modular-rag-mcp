...
def _render_ingestion_diagnostics(
    ...
    integrity_d = stages_by_name.get("integrity", {}).get("data") or {}
    error_d = stages_by_name.get("pipeline_error", {}).get("data") or {}

    if integrity_d.get("skipped"):
        st.info(
            "**Skipped — this file was already ingested into this collection.** "
            "Its content is unchanged, so nothing was reprocessed. "
            "Re-ingest with force enabled (CLI `--force`, or the force toggle on "
            "the Ingestion Manager page) to process it again."
        )

    if error_d:
        st.error(
            f"**Pipeline failed after stage "
            f"`{error_d.get('last_stage_before_error') or 'start'}`:** "
            f"`{error_d.get('error_type')}: {error_d.get('error')}`"
        )
    elif missing:
        ...
        st.warning(
            ...
            "No error was recorded, so the run was most likely interrupted. "
    ...
    img_status = load_d.get("image_extraction") if "load" in stages_by_name else None
    if img_status == "pymupdf_not_installed":
        st.warning(
            "**Images were not extracted: PyMuPDF is missing in this process.** "
            "Install it with `pip install pymupdf`, then restart the app — the "
            "availability flag is checked once at import time, so a running "
            "process never picks the package up. Until then ingestion runs "
            "text-only and image captions are silently skipped."
        )
    elif img_status:
        st.warning(f"**Image extraction failed, continuing text-only:** `{img_status}`")
