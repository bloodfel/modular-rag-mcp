...
class DataService:
    ...
    def _chroma_client(self) -> Any:
        """Return a PersistentClient for the configured persist directory."""
        from src.core.settings import load_settings, resolve_path
        import chromadb
    ...
    def list_collections(self) -> List[str]:
        """Return user-facing ChromaDB collection names.

        Benchmark corpora (``beir-*``, written by scripts/prepare_beir.py)
        are hidden — thousands of documents make the UI unusably slow and
        they are internal fixtures, not knowledge-base content.
        """
        try:
    ...
    def create_collection(self, name: str) -> None:
        ...
        client = self._chroma_client()
        client.get_or_create_collection(name=name.strip())

    def delete_collection(self, name: str) -> None:
        """Delete a collection and all of its chunks."""
        client = self._chroma_client()
