...
class IngestionPipeline:
    ...
    def __init__(
        ...
        # Stage 2: Loaders — picked per file extension in run()
        self.loader = PdfLoader(
            extract_images=True,
            image_storage_dir=str(resolve_path(f"data/images/{collection}"))
        )
        self.text_loader = TextLoader()
        logger.info("  ✓ PdfLoader + TextLoader initialized")
        ...
        try:
            ...
            if not self.force and self.integrity_checker.should_skip(file_hash):
                ...
                if trace is not None:
                    # Without this the trace stays empty, and an empty trace is
                    # indistinguishable from a crash — which is how a skipped
                    # file came to be shown as "corrupted or unsupported".
            ...
            loader = (
                self.text_loader
                if file_path.suffix.lower() in TextLoader.SUPPORTED_SUFFIXES
                else self.loader
            )
            ...
            if trace is not None:
                trace.record_stage("load", {
                    ...
                    "text_length": len(document.text),
                    "image_count": image_count,
                    # None when extraction worked (or was disabled); a string
                    # here says why images are missing.
                    "image_extraction": document.metadata.get("image_extraction"),
            ...
            if trace is not None:
                # A failure that leaves no trace can only be seen in stderr, so
                # the dashboard had to guess at a cause.
                recorded = [s.get("stage") for s in trace.stages]
                trace.record_stage("pipeline_error", {
                    "error": str(e),
                    "error_type": type(e).__name__,
                    "last_stage_before_error": recorded[-1] if recorded else None,
                })
