...
class DocumentManager:
    ...
    def list_documents(
        ...
        # Fallback: also surface chunks written outside the file pipeline
        # (MCP ingest_document, BEIR corpus prep) — those have no integrity
        # record, so group their metadata by source_path.
        try:
            raw = self.chroma.collection.get(include=["metadatas"])
            counts: Dict[str, int] = {}
            for meta in raw.get("metadatas") or []:
                if not isinstance(meta, dict):
                    continue
                sp = meta.get("source_path")
                if sp and sp not in covered_paths:
            ...
            pass  # chroma-only enumeration is best-effort; integrity list remains
