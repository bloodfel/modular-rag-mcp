...
class ChunkRefiner(BaseTransform):
    ...
    def _refine_single_chunk(
        ...
        try:
            ...
            if self.use_llm and self.llm:
                llm_refined_text = self._llm_refine(rule_refined_text, trace, label=chunk.id)
    ...
    def _llm_refine(
        ...
        Args:
            ...
            trace: Optional trace context
            label: Which chunk this call is for (recorded on the trace)
        ...
        try:
            ...
            response = self.llm.chat(
                messages, trace=trace, purpose="chunk_refiner", label=label
            )
