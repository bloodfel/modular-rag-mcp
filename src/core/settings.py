    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Settings":
        ...
        settings = cls(
            ...
            rerank=RerankSettings(
                ...
                min_score=float(rerank.get("min_score", 4.0)),
            ),
            evaluation=EvaluationSettings(
...
def _load_dotenv_file() -> None:
    ...
    Silently does nothing when the file is missing or the package is not
    installed — environment expansion below is the real gatekeeper.
    """
    try:
...
def _expand_env_vars(node: Any) -> Any:
    """Recursively expand ``${VAR}`` references in mapping values.

    Each reference is replaced by the environment variable's value.  A
    reference to an unset variable raises :class:`SettingsError` so that
    literal ``${...}`` placeholders never reach a provider API call.

    Args:
        node: Parsed YAML value (dict, list, str, or scalar).

    Returns:
        The same structure with all references expanded.

    Raises:
        SettingsError: If a referenced environment variable is not set.
    """
    ...
    if isinstance(node, str):
        def _substitute(match: "re.Match[str]") -> str:
            ...
            if value is None:
                raise SettingsError(
                    f"Environment variable not set: {var} "
                    f"(add it to .env at the repo root)"
                )
            return value

        return _ENV_VAR_PATTERN.sub(_substitute, node)
    return node
...
def load_settings(path: str | Path | None = None) -> Settings:
    ...
    String values may reference environment variables as ``${VAR}``; the
    references are expanded after loading.  Credentials therefore live in
    a gitignored ``.env`` at the repo root instead of in settings.yaml.
    ...
    settings = Settings.from_dict(_expand_env_vars(data or {}))
