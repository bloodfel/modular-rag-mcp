...
class HybridSearch:
    ...
    def search(
        ...
        if used_fallback and trace is not None:
            # A retrieval path failed and the other carried the query. Recorded
            # as its own stage so the degradation rate is countable downstream.
            trace.record_stage("retrieval_fallback", {
                "dense_failed": bool(dense_error),
                "sparse_failed": bool(sparse_error),
                "dense_error": dense_error,
                "sparse_error": sparse_error,
                "degraded_result_count": len(fused_results or []),
            })
