...
#### 3.2.2 传输协议：Stdio（默认）+ Streamable HTTP（可选）

本项目支持 **双传输模式**，按部署场景选择：

- **Stdio Transport（默认）**：Client（VS Code Copilot、Claude Desktop、ZCode 等）以子进程方式启动 Server，通过标准输入/输出交换 JSON-RPC 消息。
  - **零配置**：无需网络端口，用户只需在 Client 配置文件中指定启动命令。
  - **隐私安全**：数据不经过网络，天然适合私有知识库。
  - **实现约束**：`stdout` 仅输出合法 MCP 消息；日志统一输出至 `stderr`。
- **Streamable HTTP Transport（`--http`）**：`python -m src.mcp_server.server --http --host 0.0.0.0 --port 8000`，在 `http://<host>:<port>/mcp` 暴露 MCP 端点。
  - **远程/多客户端**：无需在本机 spawn 子进程，团队或远程 Agent 可直接连接同一服务实例。
  - **实现方式**：官方 SDK 的 `StreamableHTTPSessionManager`（stateless + JSON 响应模式），底座为 Starlette/uvicorn（MCP SDK 自带依赖）。
  - **安全提示**：面向公网部署时应置于反向代理之后并叠加鉴权（如 Bearer token / mTLS），当前版本未内置鉴权。
...
10. **阶段 J：Jev 重排接入与 BEIR 基准评测**
  - 目的：在既有 rerank 插槽接入 TypeSafe Jev（typed judgment，不产文本只输出结构化评分），并建立 BEIR 公开数据集上 4 重排器 × 3 检索器的对比基准（质量 / 速度 / 成本 / 可靠性四维度）。
11. **阶段 K：Langfuse 可观测 MVP**
  - 目的：自托管 Langfuse 栈 + traces.jsonl 导出器，把本地 trace 导入业界标准的 LLM 可观测平台，与内置 Dashboard 形成互补。
...
| ---- | ------------------- | --- | ---------- | ------------------------------ |
| A1   | 初始化目录树与最小可运行入口      | [x] | 2025-01-26 | 目录结构、配置文件、main.py 已创建          |
| A2   | 引入 pytest 并建立测试目录约定 | [x] | 2025-01-26 | pytest 配置、tests/ 目录结构、22 个冒烟测试 |
| A3   | 配置加载与校验（Settings）   | [x] | 2025-01-26 | 配置加载、校验与单元测试                   |

#### 阶段 B：Libs 可插拔层

| 任务编号 | 任务名称                        | 状态  | 完成日期       | 备注                                                        |
| ---- | --------------------------- | --- | ---------- | --------------------------------------------------------- |
| B1   | LLM 抽象接口与工厂                 | [x] | 2025-01-27 | BaseLLM + LLMFactory + 16个单元测试                            |
| B2   | Embedding 抽象接口与工厂           | [x] | 2025-01-27 | BaseEmbedding + EmbeddingFactory + 22个单元测试                |
| B3   | Splitter 抽象接口与工厂            | [x] | 2025-01-27 | BaseSplitter + SplitterFactory + 20个单元测试                  |
| B4   | VectorStore 抽象接口与工厂         | [x] | 2025-01-27 | BaseVectorStore + VectorStoreFactory + 34个单元测试            |
| B5   | Reranker 抽象接口与工厂（含 None 回退） | [x] | 2025-01-27 | BaseReranker + RerankerFactory + NoneReranker + 单元测试      |
| B6   | Evaluator 抽象接口与工厂           | [x] | 2025-01-27 | BaseEvaluator + EvaluatorFactory + CustomEvaluator + 单元测试 |
| B7.1 | OpenAI-Compatible LLM 实现    | [x] | 2025-01-28 | OpenAILLM + AzureLLM + DeepSeekLLM + 33个单元测试              |
| B7.2 | Ollama LLM 实现               | [x] | 2025-01-28 | OllamaLLM + 32个单元测试                                       |
| B7.3 | OpenAI & Azure Embedding 实现 | [x] | 2025-01-28 | OpenAIEmbedding + AzureEmbedding + 27个单元测试                |
| B7.4 | Ollama Embedding 实现         | [x] | 2025-01-28 | OllamaEmbedding + 20个单元测试                                 |
| B7.5 | Recursive Splitter 默认实现     | [x] | 2025-01-28 | RecursiveSplitter + 24个单元测试 + langchain集成                 |
| B7.6 | ChromaStore 默认实现            | [x] | 2025-01-30 | ChromaStore + 20个集成测试 + roundtrip验证                       |
| B7.7 | LLM Reranker 实现             | [x] | 2025-01-30 | LLMReranker + 20个单元测试 + prompt模板支持                        |
| B7.8 | Cross-Encoder Reranker 实现   | [x] | 2025-01-30 | CrossEncoderReranker + 26个单元测试 + 工厂集成                     |
| B8   | Vision LLM 抽象接口与工厂集成        | [x] | 2025-01-31 | BaseVisionLLM + ImageInput + LLMFactory扩展 + 35个单元测试       |
| B9   | Azure Vision LLM 实现         | [x] | 2025-01-31 | AzureVisionLLM + 22个单元测试 + mock测试 + 图片压缩                  |

#### 阶段 C：Ingestion Pipeline MVP

| 任务编号 | 任务名称                                    | 状态  | 完成日期       | 备注                                                                        |
| ---- | --------------------------------------- | --- | ---------- | ------------------------------------------------------------------------- |
| C1   | 定义核心数据类型/契约（Document/Chunk/ChunkRecord） | [x] | 2025-01-30 | Document/Chunk/ChunkRecord + 18个单元测试                                      |
| C2   | 文件完整性检查（SHA256）                         | [x] | 2025-01-30 | FileIntegrityChecker + SQLiteIntegrityChecker + 25个单元测试                   |
| C3   | Loader 抽象基类与 PDF Loader                 | [x] | 2025-01-30 | BaseLoader + PdfLoader + PyMuPDF图片提取 + 21单元测试 + 9集成测试                     |
| C4   | Splitter 集成（调用 Libs）                    | [x] | 2025-01-31 | DocumentChunker + 19个单元测试 + 5个核心增值功能                                      |
| C5   | Transform 基类 + ChunkRefiner             | [x] | 2025-01-31 | BaseTransform + ChunkRefiner (Rule + LLM) + TraceContext + 25单元测试 + 5集成测试 |
| C6   | MetadataEnricher                        | [x] | 2025-01-31 | MetadataEnricher (Rule + LLM) + 26单元测试 + 真实LLM集成测试                        |
| C7   | ImageCaptioner                          | [x] | 2025-02-01 | ImageCaptioner + Azure Vision LLM 实现 + 集成测试                               |
| C8   | DenseEncoder                            | [x] | 2025-02-01 | 批量编码+Azure集成测试                                                            |
| C9   | SparseEncoder                           | [x] | 2025-02-01 | 词频统计+语料库统计+26单元测试                                                         |
| C10  | BatchProcessor                          | [x] | 2025-02-01 | BatchProcessor + BatchResult + 20个单元测试                                    |
| C11  | BM25Indexer（倒排索引+IDF计算）                 | [x] | 2025-02-01 | BM25索引器+IDF计算+持久化+26单元测试                                                  |
| C12  | VectorUpserter（幂等upsert）                | [x] | 2025-02-01 | 稳定chunk_id生成+幂等upsert+21单元测试                                              |
| C13  | ImageStorage（图片存储+SQLite索引）             | [x] | 2025-02-01 | ImageStorage + SQLite索引 + 37个单元测试 + WAL并发支持                               |
| C14  | Pipeline 编排（MVP 串起来）                    | [x] | 2025-02-02 | 完整流程编排+Azure LLM/Embedding集成测试通过                                          |
| C15  | 脚本入口 ingest.py                          | [x] | 2025-02-02 | CLI脚本+E2E测试+文件发现+skip功能                                                   |

#### 阶段 D：Retrieval MVP

| 任务编号 | 任务名称                                 | 状态  | 完成日期       | 备注                                                              |
| ---- | ------------------------------------ | --- | ---------- | --------------------------------------------------------------- |
| D1   | QueryProcessor（关键词提取 + filters）      | [x] | 2025-02-03 | ProcessedQuery类型+关键词提取+停用词过滤+filter语法+38单元测试                    |
| D2   | DenseRetriever（调用 VectorStore.query） | [x] | 2025-02-03 | RetrievalResult类型+依赖注入+ChromaStore.query修复+30单元测试               |
| D3   | SparseRetriever（BM25 查询）             | [x] | 2025-02-04 | BaseVectorStore.get_by_ids+ChromaStore实现+SparseRetriever+26单元测试 |
| D4   | RRF Fusion                           | [x] | 2025-02-04 | RRFFusion类+k参数可配置+加权融合+确定性输出+34单元测试                             |
| D5   | HybridSearch 编排                      | [x] | 2025-02-04 | HybridSearch类+并行检索+优雅降级+元数据过滤+29集成测试                            |
| D6   | Reranker（Core 层编排 + Fallback）        | [x] | 2025-02-04 | CoreReranker+LLM Reranker集成+Fallback机制+27单元测试+7集成测试             |
| D7   | 脚本入口 query.py（查询可用）                  | [x] | 2025-02-04 | CLI 查询入口 + verbose 输出                                           |

#### 阶段 E：MCP Server 层与 Tools

| 任务编号 | 任务名称                       | 状态  | 完成日期       | 备注                                                                 |
| ---- | -------------------------- | --- | ---------- | ------------------------------------------------------------------ |
| E1   | MCP Server 入口与 Stdio 约束    | [x] | 2025-02-04 | server.py 使用官方 MCP SDK + stdio + 2集成测试                             |
| E2   | Protocol Handler 协议解析与能力协商 | [x] | 2025-02-04 | ProtocolHandler类+tool注册+错误处理+20单元测试                                |
| E3   | query_knowledge_hub Tool   | [x] | 2025-02-04 | ResponseBuilder+CitationGenerator+Tool注册+24单元测试+2集成测试              |
| E4   | list_collections Tool      | [x] | 2025-02-04 | ListCollectionsTool+CollectionInfo+ChromaDB集成+41单元测试+2集成测试         |
| E5   | get_document_summary Tool  | [x] | 2025-02-04 | GetDocumentSummaryTool+DocumentSummary+错误处理+71单元测试                 |
| E6   | 多模态返回组装（Text + Image）      | [x] | 2025-02-04 | MultimodalAssembler+base64编码+MIME检测+ResponseBuilder集成+54单元测试+4集成测试 |

#### 阶段 F：Trace 基础设施与打点

| 任务编号 | 任务名称                                        | 状态  | 完成日期       | 备注                                                                         |
| ---- | ------------------------------------------- | --- | ---------- | -------------------------------------------------------------------------- |
| F1   | TraceContext 增强（finish + 耗时统计 + trace_type） | [x] | 2025-02-08 | TraceContext增强(trace_type/finish/elapsed_ms/to_dict)+TraceCollector+28单元测试 |
| F2   | 结构化日志 logger（JSON Lines）                    | [x] | 2025-02-08 | JSONFormatter+get_trace_logger+write_trace+16单元测试                          |
| F3   | 在 Query 链路打点                                | [x] | 2025-02-08 | HybridSearch+CoreReranker trace注入(5阶段)+14集成测试                              |
| F4   | 在 Ingestion 链路打点                            | [x] | 2025-02-08 | Pipeline五阶段trace注入(load/split/transform/embed/upsert)+11集成测试               |
| F5   | Pipeline 进度回调 (on_progress)                 | [x] | 2025-02-08 | on_progress回调(6阶段通知)+6单元测试                                                 |

#### 阶段 G：可视化管理平台 Dashboard

| 任务编号 | 任务名称                 | 状态  | 完成日期       | 备注                                                                               |
| ---- | -------------------- | --- | ---------- | -------------------------------------------------------------------------------- |
| G1   | Dashboard 基础架构与系统总览页 | [x] | 2025-02-09 | app.py多页面导航+overview页+ConfigService+start_dashboard.py+11单元测试                    |
| G2   | DocumentManager 实现   | [x] | 2025-02-09 | DocumentManager跨存储协调(ChromaStore+BM25+ImageStorage+IntegrityChecker)+文档删除+21单元测试 |
| G3   | 数据浏览器页面              | [x] | 2025-02-09 | DataService只读门面+文档列表+chunk内容展示+元数据JSON展开+collection切换                            |
| G4   | Ingestion 管理页面       | [x] | 2025-02-09 | 文件上传+IngestionPipeline集成+实时进度条+TraceContext自动记录                                  |
| G5   | Ingestion 追踪页面       | [x] | 2025-02-09 | TraceService读取traces.jsonl+阶段时间线+耗时柱状图+stage详情展开                                 |
| G6   | Query 追踪页面           | [x] | 2025-02-09 | Query trace过滤+检索结果展示+rerank对比+耗时分析                                               |

#### 阶段 H：评估体系

| 任务编号 | 任务名称                         | 状态  | 完成日期       | 备注                                                          |
| ---- | ---------------------------- | --- | ---------- | ----------------------------------------------------------- |
| H1   | RagasEvaluator 实现            | [x] | 2025-02-09 | 19/19 tests passed                                          |
| H2   | CompositeEvaluator 实现        | [x] | 2025-02-09 | 11/11 tests passed                                          |
| H3   | EvalRunner + Golden Test Set | [x] | 2025-02-09 | 15/15 tests passed                                          |
| H4   | 评估面板页面                       | [x] | 2025-02-09 | 6/6 tests passed, dashboard page with history tracking      |
| H5   | Recall 回归测试（E2E）             | [x] | 2025-02-09 | 3 unit+4 e2e(skip without data), hit@k+MRR threshold gating |

#### 阶段 I：端到端验收与文档收口

| 任务编号 | 任务名称                              | 状态  | 完成日期       | 备注                                        |
| ---- | --------------------------------- | --- | ---------- | ----------------------------------------- |
| I1   | E2E：MCP Client 侧调用模拟              | [x] | 2025-02-23 | 7个E2E测试+import死锁修复+非阻塞readline            |
| I2   | E2E：Dashboard 冒烟测试                | [x] | 2025-02-24 | 6个页面冒烟测试+AppTest框架+mock服务                 |
| I3   | 完善 README（运行说明 + MCP + Dashboard） | [x] | 2025-02-24 | 快速开始+配置说明+MCP配置+Dashboard指南+测试+FAQ        |
| I4   | 清理接口一致性（契约测试补齐）                   | [x] | 2025-02-24 | VectorStore+Reranker+Evaluator边界测试+83测试全绿 |
| I5   | 全链路 E2E 验收                        | [x] | 2025-02-24 | 1198单元+30e2e通过,ingest/query/evaluate脚本验证  |

#### 阶段 J：Jev 重排接入与 BEIR 基准评测

| 任务编号 | 任务名称                              | 状态  | 完成日期       | 备注                                                        |
| ---- | --------------------------------- | --- | ---------- | --------------------------------------------------------- |
| J1   | BEIR 评测指标库（nDCG/MRR/Hit@k）        | [x] | 2026-09-21 | src/observability/evaluation/metrics.py + 10单元测试           |
| J2   | JevReranker provider（System One API） | [x] | 2026-09-21 | httpx直连+payload校准+min_score过滤+_jev_meta用量+8单元测试          |
| J3   | 工厂注册 + 配置扩展（${ENV} 展开）            | [x] | 2026-09-21 | reranker_factory注册"jev"+settings支持${VAR}环境变量+.env.example |
| J4   | BEIR 数据准备脚本（scifact/nfcorpus 灌库）  | [x] | 2026-09-21 | prepare_beir.py+幂等灌库+smoke隔离+双存储(Chroma+BM25)             |
| J5   | 基准跑批脚本 + 四维报告                     | [x] | 2026-09-22 | benchmark_rerank.py+查询级并发+bge串行修复(torch MPS)+成本/可靠性维度     |

#### 阶段 K：Langfuse 可观测 MVP

| 任务编号 | 任务名称                              | 状态  | 完成日期       | 备注                                                        |
| ---- | --------------------------------- | --- | ---------- | --------------------------------------------------------- |
| K1   | Langfuse 自托管栈（docker compose）     | [x] | 2026-09-22 | infra/langfuse/ v4 全家桶(web/worker/pg/clickhouse/redis/minio) |
| K2   | traces.jsonl → Langfuse 导出器       | [x] | 2026-09-22 | scripts/export_langfuse.py trace→span映射+--last/--type CLI  |
| K3   | 导出器单元测试 + 复跑 Runbook              | [x] | 2026-09-22 | 5单元测试+docs/specs/2026-09-21-langfuse-mvp-design.md 末尾Runbook |
...
- **验收标准**：
  ...
  - 支持配置 Ollama 服务地址（默认 [http://localhost:11434）和模型名称。](http://localhost:11434）和模型名称。)
...
## 阶段 J：Jev 重排接入与 BEIR 基准评测（目标：用公开数据集量化每种重排路线）

> 详细设计见 `docs/specs/2026-09-21-jev-rerank-benchmark-design.md`（含进度跟踪与验收清单）。

### J1：BEIR 评测指标库

- **目标**：实现 `ndcg_at_k / mrr_at_k / hit_at_k` 纯函数指标库（2^rel−1 增益、理想 DCG 归一）。
- **修改文件**：`src/observability/evaluation/metrics.py`、`tests/unit/test_metrics.py`。
- **验收标准**：与手工算例一致；空排序列返回 0。
- **测试方法**：`pytest -q tests/unit/test_metrics.py`。

### J2：JevReranker provider

- **目标**：直连 TypeSafe System One API（`jev-1.13.0`）——一次调用对全部候选输出 typed Score + 校准概率，无文本生成、无 JSON 解析失败路径；支持 `min_score` 阈值过滤与 `_jev_meta` 用量记录。
- **修改文件**：`src/libs/reranker/jev_reranker.py`、`tests/unit/test_jev_reranker.py`。
- **验收标准**：候选 ≤10（API 硬约束）；缺答案/非数值评分 fail-loud 抛错；凭证仅从 `TYPESAFE_API_KEY` 环境变量读取。
- **测试方法**：`pytest -q tests/unit/test_jev_reranker.py`（mock `_post`，无真实网络）。

### J3：工厂注册与配置扩展

- **目标**：`RerankerFactory` 注册 `"jev"`；`settings.yaml` 支持 `${ENV_VAR}` 占位符展开（从仓库根 `.env` 加载），凭证不入库。
- **修改文件**：`src/libs/reranker/reranker_factory.py`、`src/core/settings.py`、`.env.example`。
- **验收标准**：改一行配置即可在 none / cross_encoder / llm / jev 间切换；缺失环境变量报 `SettingsError`。
- **测试方法**：`pytest -q tests/unit/ -k settings`。

### J4：BEIR 数据准备脚本

- **目标**：`ir_datasets` 语料灌入项目自有存储（Chroma 集合 `beir-<name>` + BM25 索引），queries/qrels 落盘 `data/beir/<name>/`；`--limit` smoke 模式完全隔离。
- **修改文件**：`scripts/prepare_beir.py`。
- **验收标准**：幂等（`num_docs == docs_count` 跳过）；smoke 产物不污染正式库。
- **测试方法**：`python scripts/prepare_beir.py --datasets scifact --limit 20` 后核对集合计数。

### J5：基准跑批脚本与四维报告

- **目标**：`benchmark_rerank.py` 在 BEIR **test split** 固定抽样（seed=42）上对比 `none/bge/jev/llm × bm25/dense/hybrid`，输出 nDCG@10 / MRR@10 / Hit@10、延迟 p50/p95、估算成本、失败回退率的 Markdown + JSON 报告。
- **关键工程决策**：查询级线程并发（限流退避）；本地 bge 强制串行——torch 2.14 在 Apple Silicon MPS 后端下并发提交推理会原生段错误（exit 133/139）。
- **修改文件**：`scripts/benchmark_rerank.py`、`reports/`。
- **验收标准**：全程真实 API 无 mock；重跑 seed=42 结果可复现；报告含小样本警示与语料截断 caveat。
- **测试方法**：`python scripts/benchmark_rerank.py --datasets scifact --num-queries 2` 冒烟后跑全量。

## 阶段 K：Langfuse 可观测 MVP（目标：本地 trace 导入业界标准平台）

> 详细设计与复跑 Runbook 见 `docs/specs/2026-09-21-langfuse-mvp-design.md`。

### K1：Langfuse 自托管栈

- **目标**：docker compose 一键拉起 Langfuse v4 全家桶（web / worker / postgres / clickhouse / redis / minio），本地 `localhost:3000` 可用。
- **修改文件**：`infra/langfuse/docker-compose.yml`（密钥走 `infra/langfuse/.env`，gitignore）。
- **验收标准**：`docker compose ps` 全 healthy。

### K2：trace 导出器

- **目标**：`scripts/export_langfuse.py` 读取 `logs/traces.jsonl`，把一次查询/摄取 trace 映射为 Langfuse trace + 分阶段 span（span 结束时间为下一阶段开始），批量上报。
- **验收标准**：纯函数映射可单测；`--last N --type query` CLI 可用。

### K3：导出器单元测试与 Runbook

- **目标**：映射逻辑 5 个单测；spec 文档末尾附"从零复跑"Runbook（起栈 → 注册 → 建 key → 查询 → 导出）。
- **测试方法**：`pytest -q tests/unit/test_export_langfuse.py`。
...
- **M7（完成阶段 J+K）**：重排路线有公开数据集量化对比（质量/速度/成本/可靠性），trace 可导入 Langfuse 做业界标准可观测。
