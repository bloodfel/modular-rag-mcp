...
Usage:
    ...
    # Ingest Markdown/txt files from a directory (golden-set corpus)
    python scripts/ingest.py --path docs/ --collection project_docs --extensions .md
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="File extensions to include when --path is a directory "
             "(default: .pdf). Text formats .md/.markdown/.txt are also "
             "supported, e.g. --extensions .pdf .md"
    )
...
def discover_files(path: str, extensions: List[str] = None) -> List[Path]:
    ...
    if extensions is None:
        extensions = ['.pdf']
    # Accept ".md"/"md"/"MD" alike — CLI users omit the dot half the time.
    extensions = [
...
def main() -> int:
    ...
    # Discover files
    try:
        files = discover_files(args.path, extensions=args.extensions)
